...
class BallController:
    ...
    def update_sphere(self) -> None:
        ...
        # ramp_angle = math.radians(89)  # Ângulo de inclinação da rampa (ajuste conforme necessário)
        ...
        # self.__ball_position.sphere_velocity[0] += ramp_force
        # self.__ball_position.sphere_velocity[1] += ramp_force
        ...
        # self.__ball_position.sphere_position[2] += self.__ball_position.sphere_velocity[1]
        ...
        # self.__ball_position.sphere_velocity[2] += self.__ball_position.sphere_velocity[0] * self.__atrator  # Aplica a força gravitacional na direção x crescente
        # self.__ball_position.sphere_position[2] += self.__ball_position.sphere_velocity[2]  # Atualiza a posição da esfera na direção x
        
        
        # if self.__ball_position.sphere_position[0] > 0:
        #     self.__ball_position.gravity = -0.1
        # elif self.__ball_position.sphere_position[0] < 0:
        #     self.__ball_position.gravity = 0.1
        ...
        self.__damping_factor = 0.95 if self.__ball_position.sphere_velocity[0] > 0 else 0.98
        ...
        print(self.__ball_position.sphere_position)
        # print(self.__ball_position.sphere_velocity[0])
        glut.glutPostRedisplay()    
    
    #bumpers = [(5, 0, 1), (-5, 0, 1), (0, 5, 1), (0, -5, 1)]
    ...
    #     print(self.__ball_position.sphere_position[0])
    #     # Verifica se a posição da esfera está dentro das dimensões do objeto
    #     if self.__ball_position.sphere_position[0] >= self.object_left or self.__ball_position.sphere_position[0] <= self.object_right:
    #         #if self.__ball_position.sphere_position[2] >= self.object_bottom or self.__ball_position.sphere_position[2] <= self.object_top:
    #             # Inverte a direção da velocidade da esfera na direção x
    #             print('g')
    #             self.__ball_position.sphere_velocity[0] = -self.__ball_position.sphere_velocity[0]
    ...
    #             self.__ball_position.sphere_velocity[1] *= -1  # Inverte a direção da velocidade na direção z
    ...
    #             self.__ball_position.sphere_position[2] += self.__ball_position.sphere_velocity[1] * 0.1
    ...
    #             self.__ball_position.sphere_velocity = np.dot(rotation_matrix, self.__ball_position.sphere_velocity)
    ...
    #             # Rotaciona a bola em 45 graus em torno do eixo y
    #             collision_direction = np.array([self.__ball_position.sphere_velocity[0], 0, self.__ball_position.sphere_velocity[1]])
    ...
    #             self.__ball_position.sphere_velocity[1] = new_direction[2]
    ...
    def check_bumper_collision(self) -> None:
        for bumper in self.bumpers:
            ...
            if distance < bumper_radius:
                ...
                # A colisão ocorreu
                #self.__ball_position.sphere_velocity[2] += self.__ball_position.gravity
                #print(distance)
                # Ajusta a velocidade da esfera para simular o efeito de retorno
                self.__ball_position.sphere_velocity[0] *= -1  # Inverte a direção da velocidade na direção x
                ...
                # Atualiza a posição da esfera para evitar que fique presa dentro do bumper
                self.__ball_position.sphere_position[0] += self.__ball_position.sphere_velocity[0] * 0.1
                self.__ball_position.sphere_position[2] += self.__ball_position.sphere_velocity[2] * 0.1
